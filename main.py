...
def export_results(model_name: str = None, output_file: str = "ethics_results.json"):
...
def run_web_dashboard(port: int = 8050, debug: bool = False):
...
def show_stance_changes(model_name: str = None, alert_level: str = None):
    ...
    for change in changes[:10]:  # Show latest 10
        ...
        print(f"     {change.previous_stance.value} → {change.new_stance.value}")
        print(f"     Magnitude: {change.magnitude:.2f} ({change.alert_level} alert)")
        print(f"     Changed: {change.new_timestamp.strftime('%Y-%m-%d %H:%M')}")
        print()
...
Examples:
  ...
  python main.py web --port 8080               # Launch web dashboard
        """
    )
    ...
    test_parser.add_argument('--model', default='mock-gpt-4', 
    ...
    changes_parser.add_argument('--alert', choices=['low', 'medium', 'high'],
    ...
    export_parser.add_argument('--output', default='ethics_results.json',
